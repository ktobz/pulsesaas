...
function LoginForm() {
  ...
  useEffect(() => {
    fetch("/api/auth/providers")
      .then((r) => r.json())
      .then((data) => setHasGoogle(!!data.google))
  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh]">
      <div className="w-full max-w-md border border-[var(--border)] rounded-xl p-8">
        ...
        {errorParam && (
          <div className="p-3 rounded-lg mb-4 text-sm bg-amber-50 text-amber-700 dark:bg-amber-900 dark:text-amber-200">
            {errorParam === "OAuthAccountNotLinked"
              ? "This email is already registered with a different sign-in method."
              : errorParam === "OAuthSignin"
              ? "Google OAuth not configured. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env.local, or sign in with email/password."
